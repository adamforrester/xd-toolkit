...
export async function initCommand(opts) {
  ...
  // 5. assets/ directory — drop folder for client brand assets (PDFs, screenshots, logos).
  // Mirrors brand-cli init's behavior so practitioners using xd-toolkit get the full workflow
  // without having to run brand-cli init separately.
  ...
  // Note: brand.md, design.md, and .impeccable.md are no longer generated at init.
  // They're regenerated by the brand-skills pipeline (`/brand-context:extract` skill or
  // `brand-cli refresh-design` / `brand-cli refresh-context` CLI). Until that runs,
  // agents read .brand/ directly.
  ...
  console.log(`  2. In Claude Code, run ${chalk.cyan('/brand-context:extract')} — the skill scans assets, asks for any URLs (website, Figma, social), populates .brand/, and generates design.md + brand.md`);
  console.log(`  3. Review .brand/ files and refine; re-run ${chalk.cyan('/brand-context:extract')} as needed (additive on voice/conflicts)`);
