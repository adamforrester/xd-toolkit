...
docs/
  ...
  daily-workflow.md         # End-to-end "what a typical project looks like day to day"
  test-results.md           # Layer 1-5 test findings (Layers 1, 3 fully passed; 2, 4 pass with caveats; 5 unblocked, pending)
```
...
| C2 | `xd-toolkit` CLI | **Done** | setup, init, doctor, update commands. 22 skills bundled. Brand auditing/scoring (`refresh-design`, `refresh-impeccable`, `score`) all moved to the standalone `brand-cli` (from the brand-skills repo). |
...
| C6 | `/brand-score` skill | **Removed from xd-toolkit** | Brand auditing/scoring is now owned entirely by the brand-skills repo: `brand-cli score` (completeness) and `/brand-context:audit` (adherence). The legacy `xd-toolkit score` CLI command was removed; xd-toolkit no longer participates in this surface. |
...
- **Layer 2 (Impeccable Integration):** Pass — build output on-brand. Briefs are scope-focused by design (planning captures scope, build applies brand); the earlier "partial pass" framing read this as a defect when it's the intended division of labor.
...
- **Layer 5 (E2E):** Unblocked, execution pending. C7 (`/brand-context:audit`) shipped in brand-skills v0.4.0, so the full prototype-build run-through (`/brand-extract` → Impeccable build → `/brand-context:audit`) is now possible. Stage 6 DS repo scan validation is parked with the brand-skills agent separately.
...
2. **Brand Skills** — Now owned by the standalone brand-skills repo. `/brand-extract` shipped at v1.0.0 (C4 covers C5/C8 inline). C7 (`/brand-context:audit`) shipped in brand-skills v0.4.0. C6 removed from xd-toolkit — brand-skills owns auditing/scoring entirely. — **DONE (xd-toolkit side)**
3. **Distribution** — Public repo live. Daily-workflow doc shipped. Broader practitioner testing remains.
